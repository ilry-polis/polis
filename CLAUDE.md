...
- Watch the Polis monitor. At **WARNING (~41%)** start delegating and prefer
  short tasks; at **HIGH (~61%+)** finish and commit the current task only; at
  **CRITICAL (~75%+)** run `/polis:pause-work`, then `/compact`, then
  `/polis:resume-work`.
- The percentage is a conservative estimate (the runtime doesn't expose the real
  figure) — trust the direction more than the exact number.
...
`/polis:init` `/polis:discuss` `/polis:spec` `/polis:plan` `/polis:exec`
`/polis:review` `/polis:verify` `/polis:status` `/polis:next` `/polis:health`
`/polis:config` `/polis:pause-work` `/polis:resume-work`
